...
func desTest() {
	...
	fmt.Println("加密后明文：", enc, conv.ToString(enc))
...
// EnTxtByAesWithErr 加密base64
func EnTxtByAesWithErr(pwdStr, PwdKey string) (string, error) {
	if len(PwdKey) == 0 {
		PwdKey = DefaultTxtKey
	}
	pwd := []byte(pwdStr)
	...
	//fmt.Println("EnTxtByAes:", err)
	if err != nil {
		logs.Errorf("EnTxtByAes err, %s", err)
		return "", err
	}
	return hex.EncodeToString(result), nil
}

// DeTxtByAesWithErr 解密base64 aes
func DeTxtByAesWithErr(pwd, PwdKey string) (string, error) {
	if len(PwdKey) == 0 {
		PwdKey = DefaultTxtKey
	}
	temp, _ := hex.DecodeString(pwd)
	...
	if err != nil {
		logs.Errorf("DeTxtByAes err, %s", err)
		return "", err
	}
	return res, nil
}
